...
public class ExtentCucumberFormatter implements Reporter, Formatter {
    ...
    public ExtentCucumberFormatter(File filePath) {
        if (!filePath.getPath().equals("")) {
            ...
            ExtentCucumberFormatter.htmlReportDir = new File(reportPath);
            ExtentCucumberFormatter.extent = new ExtentReports(reportPath);
        } else {
            String reportDir = "output/Run_" + System.currentTimeMillis();
            ExtentCucumberFormatter.htmlReportDir = new File(reportDir);
            ExtentCucumberFormatter.extent = new ExtentReports(reportDir + "/report.html");
        }
    }
    ...
    public static void initiateExtentCucumberFormatter() {
        String reportFilePath = "ExtentReports" + File.separator + "Run_" + System.currentTimeMillis() + File.separator +
                "report.html";
    ...
    private OutputStream reportFileOutputStream(String fileName) {
        try {
            return new URLOutputStream(new URL(ExtentCucumberFormatter.htmlReportDir.toURI().toURL(), fileName));
